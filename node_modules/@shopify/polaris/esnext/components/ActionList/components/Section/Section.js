...
export default function Section({ section, hasMultipleSections, actionRole, onActionAnyItem, }) {
    ...
    const actionMarkup = section.items.map((_a, index) => {
        var { content, onAction } = _a, item = tslib_1.__rest(_a, ["content", "onAction"]);
        return (<Item key={`${content}-${index}`} content={content} role={actionRole} onAction={handleAction(onAction)} {...item}/>);
    });
