module.exports = {
    processPayment: async (ctx, next) => {
        if (ctx.query.charge_id) {
            ...
            fetch(`https://${ctx.session.shop}/admin/recurring_application_charges/${ctx.query.charge_id}.json`, optionsWithGet)
                ...
                .then((myJson) => {
                     if (myJson.recurring_application_charge.status === 'accepted') {
                         ...
                         fetch(`https://${ctx.session.shop}/admin/recurring_application_charges/${ctx.query.charge_id}/activate.json`, optionsWithJSON)
                             .then((response) => response.json())
